 ...
 * V00857268
 * 
 * Note that i modified I version of the KMP search algorithm in order to process
 * the string. Otherwise I also imported from the algs4.jar and used the KMP algorithm
...
public class WordSearch {
	...
	public KMP kmp;		 // Using a global KMP value in order to find the dfa[][]
	...
	*/
    public WordSearch(char[] word) {
		...
		//Going to be using the KMP algorithm to determine the KFA. Need to switch back to a string
    	//instead of a char array to do this
    	
    	String wordString = String.valueOf(word);
    	//System.out.println("Test String: " + wordString);
		kmp = new KMP(wordString);
		patternLength = wordString.length();
	...
	*/
    public boolean search(char[][] puzzle) {
    	...
    	{
    		pattern = pattern + String.valueOf(puzzle[i]);
    	}
    	
    	//System.out.println("Test String: " + pattern);
    	
    	int offset = kmp.search(pattern);
    	
    	//System.out.println("Pattern Offset: " + offset);
    	
    	if (offset >= rowLength * colLength - patternLength)
    		return false;
		
    	startRow = offset / 10;
		startCol = offset % 10;
		
		//Need to process the end rows and end column
		
		int endingValue = offset + patternLength - 1;
		
		endRow = endingValue / 10;
		endCol = endingValue % 10;
